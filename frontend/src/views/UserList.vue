<template>
  <div class="flex min-h-screen">
    <Sidebar /> <!-- user prop gönderilmiyor -->
    ...
    <div class="flex-1 p-6 bg-gray-100">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <p class="text-sm text-gray-500 mt-2">📍 {{ user.address.street }}, {{ user.address.city }}</p>
          <p class="text-sm text-gray-500">🏢 {{ user.company.name }}</p>
          <p class="text-sm text-blue-600 underline">🌐 {{ user.website }}</p>
        </div>
      </div>
    </div>
  </div>
